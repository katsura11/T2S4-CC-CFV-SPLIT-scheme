                                          ...
                                          FLOAT t0, FLOAT thf, FLOAT t1) 
{
  ...
  while (tid < N_points) {
    ...
    /**************************************** step 1 *******************************************/
    x_star1 = v_d_xhf[tid] - 0.5 * dt * d_Vx(v_d_xhf[tid], v_d_yhf[tid], thf);
    v_d_xhf_bar1[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (d_Vx(v_d_xhf[tid], v_d_yhf[tid], thf) + d_Vx(x_star1, v_d_yhf[tid], t0));

    /**************************************** step 2 *******************************************/
    y_star2 = v_d_yhf[tid] - dt * d_Vy(v_d_xhf[tid], v_d_yhf[tid], t1);
    v_d_yhf_bar2[tid] = v_d_yhf[tid] 
                        -  0.5 * dt * (d_Vy(v_d_xhf[tid], v_d_yhf[tid], t1) + d_Vy(v_d_xhf[tid], y_star2, t0));    
    
    /**************************************** step 3 *******************************************/
    x_star3 = v_d_xhf[tid] - 0.5 * dt * d_Vx(v_d_xhf[tid], v_d_yhf[tid], t1);
    v_d_xhf_bar3[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (d_Vx(v_d_xhf[tid], v_d_yhf[tid], t1) + d_Vx(x_star3, v_d_yhf[tid], thf));

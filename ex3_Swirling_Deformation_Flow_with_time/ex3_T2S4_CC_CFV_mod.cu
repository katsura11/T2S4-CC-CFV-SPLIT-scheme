...
__host__ __device__ FLOAT Vx(FLOAT x, FLOAT y, FLOAT t) {
  return -2*M_PI*cos(x/2)*cos(x/2)*sin(y)*cos(M_PI*t/Tf);
}

__host__ __device__ FLOAT Vy(FLOAT x, FLOAT y, FLOAT t) {
...
__host__ __device__ FLOAT d_fIntC(FLOAT vC6, FLOAT vDltc, FLOAT vCL, FLOAT xi) {
  return -1.0 / 3 * vC6 * pow(xi, 3) + 1.0 / 2 * (vDltc + vC6) * pow(xi, 2) +
         vCL * xi;
}
...
/************************************ kernel functions ************************************/
// calculate solution
__global__ void Get_initial_solution_on_gpu(FLOAT *v_d_x, FLOAT *v_d_y, FLOAT *v_C0, 
                                          FLOAT dx, FLOAT dy, INT N_grid) 
{
  ...
  while (tid < N_grid) {
    ...
    v_C0[tid] = 1.0/ 36.0 * 
                    (C0(x_mhf, y_mhf) + C0(x_phf, y_mhf) + 
                     C0(x_mhf, y_phf) + C0(x_phf, y_phf) + 
                     4.0 * ((C0(x_mhf, y) + C0(x_phf, y) + 
                            C0(x, y_mhf) + C0(x, y_phf))) + 
                     16.0 * C0(x, y));
                                          ...
                                          FLOAT t0, FLOAT thf, FLOAT t1) 
{
  ...
  while (tid < N_points) {
    ...
    /**************************************** step 1 *******************************************/
    x_star1 = v_d_xhf[tid] - 0.5 * dt * Vx(v_d_xhf[tid], v_d_yhf[tid], thf);
    v_d_xhf_bar1[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid], thf) + Vx(x_star1, v_d_yhf[tid], t0));

    /**************************************** step 2 *******************************************/
    y_star2 = v_d_yhf[tid] - dt * Vy(v_d_xhf[tid], v_d_yhf[tid], t1);
    v_d_yhf_bar2[tid] = v_d_yhf[tid] 
                        -  0.5 * dt * (Vy(v_d_xhf[tid], v_d_yhf[tid], t1) + Vy(v_d_xhf[tid], y_star2, t0));    
    
    /**************************************** step 3 *******************************************/
    x_star3 = v_d_xhf[tid] - 0.5 * dt * Vx(v_d_xhf[tid], v_d_yhf[tid], t1);
    v_d_xhf_bar3[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid], t1) + Vx(x_star3, v_d_yhf[tid], thf));
...
int main(int argc, char *argv[]) {
  ...
  *************************************************/
  ...
  Get_initial_solution_on_gpu<<<numBlocks, threadsPerBlock>>>(v_d_x, v_d_y, v_d_C0, dx, dy, N_grid);
  ...
  //for (itime = 1; itime <= 1; itime++){
    ...
    (v_d_xhf, v_d_yhf, v_d_xhf_bar1, v_d_yhf_bar2, v_d_xhf_bar3, Nx, Ny, N_points, dx, dy, dt, t0, thf, t1); 

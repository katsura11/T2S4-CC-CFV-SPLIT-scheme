...
/************************************ inline functions ************************************/
__host__ __device__ inline FLOAT Vx(FLOAT x, FLOAT y) { return 0; }
__host__ __device__ inline FLOAT Vy(FLOAT x, FLOAT y) { return 0; }
...
__host__ __device__ FLOAT d_fIntC(FLOAT vC6, FLOAT vDltc, FLOAT vCL, FLOAT xi) {
...
__host__ __device__ FLOAT d_x_sum(FLOAT *v_d_C0, INT iL, INT iR, INT j, INT Ny) {
  FLOAT sum = 0.0;
  for (int i_cell = iL; i_cell <= iR; i_cell++) {
    sum += v_d_C0[i_cell * Ny + j];
  }
  return sum;
}
__host__ __device__ FLOAT d_y_sum(FLOAT *v_d_C0, INT jL, INT jR, INT i, INT Ny) {
  FLOAT sum = 0;
  for (int j_cell = jL; j_cell <= jR; j_cell++) {
    sum += v_d_C0[i* Ny + j_cell];
  }
  return sum;
}
...
__global__ void x_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        ...
                                        FLOAT K, FLOAT dt, FLOAT dt_mul_dx_inverse, 
...
__global__ void y_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        ...
                                        FLOAT K, FLOAT dt, FLOAT dt_mul_dy_inverse, 
...
int main(int argc, char *argv[]) {
  ...
  *************************************/
  ...
  FLOAT *v_d_IntC_xphf, *v_d_DIntC_xphf_bar, *v_d_Cp1, *v_d_C_Exact;  // Variables for the cell values in step 3
  ...
  *************************************************/
  ...
  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock>>>(v_d_x, v_d_y, v_d_C0, dx, dy, 0, K, N_grid);
  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock>>>(v_d_x, v_d_y, v_d_C_Exact, dx, dy, T_max, K, N_grid);
  cudaMemcpy(v_h_C0, v_d_C0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
  cudaMemcpy(v_h_C_Exact, v_d_C_Exact, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
  ...
  //for (itime = 1; itime <= 1; itime++){
    ...
    N_grid, Nx, K, dt, dt_mul_dx_inverse, dt_mul_dx_square_inverse);
    cudaDeviceSynchronize();
    ...
    N_grid, Ny, K, dt, dt_mul_dy_inverse, dt_mul_dy_square_inverse);
    cudaDeviceSynchronize();
    ...
    //****************** Diffusion term *****************
    x_Diff_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_IntC_yp1, v_d_DIntC_xphf_bar, v_d_xhf, 

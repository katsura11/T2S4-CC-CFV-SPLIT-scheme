...
__host__ __device__ inline FLOAT d_fIntC(FLOAT vC6, FLOAT vDltc, FLOAT vCL, FLOAT xi) {
...
__host__ __device__ FLOAT d_x_sum(FLOAT *v_C0, INT iL, INT iR, INT j, INT Ny) {
  FLOAT sum = 0.0;
...
__host__ __device__ FLOAT d_y_sum(FLOAT *v_C0, INT jL, INT jR, INT i, INT Ny) {
  FLOAT sum = 0;
...
__global__ void Get_exact_solution_on_gpu(FLOAT *v_d_x, FLOAT *v_d_y, FLOAT *v_C_Exact, 
                                          FLOAT dx, FLOAT dy, FLOAT t, FLOAT K, INT N_grid) 
{
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  FLOAT x, y, x_mhf, x_phf, y_mhf, y_phf;
  while (tid < N_grid) {
    x = v_d_x[tid];
    y = v_d_y[tid];
    x_mhf = x - 0.5 * dx;
    ...
    v_C_Exact[tid] = 1.0/ 36.0 * 
                    (C_Exact(x_mhf, y_mhf, t, K) + C_Exact(x_phf, y_mhf, t, K) + 
                     C_Exact(x_mhf, y_phf, t, K) + C_Exact(x_phf, y_phf, t, K) + 
                     4.0 * ((C_Exact(x_mhf, y, t, K) + C_Exact(x_phf, y, t, K) + 
                            C_Exact(x, y_mhf, t, K) + C_Exact(x, y_phf, t, K))) + 
                     16.0 * C_Exact(x, y, t, K));
...
__global__ void get_source_term_on_gpu(FLOAT *v_d_x, FLOAT *v_d_y, FLOAT *v_d_f, 
                                      FLOAT dx, FLOAT dy, FLOAT t, FLOAT K, INT N_grid) 
{
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  FLOAT x, y, x_mhf, x_phf, y_mhf, y_phf;
  while (tid < N_grid) {
    ...
    v_d_f[tid] = 1.0 / 36.0 * 
                (f(x_mhf, y_mhf, t, K) + f(x_phf, y_mhf, t, K) + 
                  f(x_mhf, y_phf, t, K) + f(x_phf, y_phf, t, K) + 
                  4.0 * ((f(x_mhf, y, t, K) + f(x_phf, y, t, K) + 
                        f(x, y_mhf, t, K) + f(x, y_phf, t, K))) + 
                  16.0 * f(x, y, t, K));
                                          ...
                                          INT N_points, FLOAT dx, FLOAT dy, FLOAT dt) 
{
  ...
  while (tid < N_points) {
    ...
    /**************************************** step 1 *******************************************/
    x_star1 = v_d_xhf[tid] - 0.5 * dt * Vx(v_d_xhf[tid], v_d_yhf[tid]);
    v_d_xhf_bar1[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid]) + Vx(x_star1, v_d_yhf[tid]));

    /**************************************** step 2 *******************************************/
    y_star2 = v_d_yhf[tid] -       dt * Vy(v_d_xhf[tid], v_d_yhf[tid]);
    v_d_yhf_bar2[tid] = v_d_yhf[tid] 
                        -  0.5 * dt * (Vy(v_d_xhf[tid], v_d_yhf[tid]) + Vy(v_d_xhf[tid], y_star2));    
    
    /**************************************** step 3 *******************************************/
    x_star3 = v_d_xhf[tid] - 0.5 * dt * Vx(v_d_xhf[tid], v_d_yhf[tid]);
    v_d_xhf_bar3[tid] = v_d_xhf[tid] 
                        - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid]) + Vx(x_star3, v_d_yhf[tid]));
...
__global__ void x_Diff_on_gpu(FLOAT *v_d_CL, FLOAT *v_d_CR, FLOAT *v_d_C,
                              ...
                              FLOAT *v_d_x_inter1, FLOAT K, INT N_points_inter1, 
                              INT Nx, INT Ny, FLOAT dx) {
  int ind = blockDim.x * blockIdx.x + threadIdx.x;
  int i_cell, j_cell;
  FLOAT one_elev = 1.0 / 11.0;
...
__global__ void y_Diff_on_gpu(FLOAT *v_d_CL, FLOAT *v_d_CR, FLOAT *v_d_C, FLOAT *v_d_DC_y_bar, 
                              FLOAT *v_d_yhf, FLOAT *v_d_y_inter2, FLOAT K, INT N_points_inter2, 
...
__global__ void x_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        FLOAT *v_d_CRemap, FLOAT *v_d_DC_x_bar, FLOAT *v_d_f_p0, 
                                        ...
                                        FLOAT dt_mul_dx_inverse, FLOAT dt_mul_dx_square_inverse) 
{
  ...
  while (ind < N_grid) {
    ...
    v_d_b[ind] = v_d_CRemap[ind] + 0.25 * dt * (v_d_f_p0[ind] + v_d_f_p1[ind])
...
__global__ void y_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        FLOAT *v_d_CRemap, FLOAT *v_d_DC_y_bar, FLOAT *v_d_f_p0, 
                                        FLOAT *v_d_f_p1, INT N_grid, INT Ny, FLOAT K, FLOAT dt, 
                                        FLOAT dt_mul_dy_inverse, FLOAT dt_mul_dy_square_inverse) 
{
  ...
  while (ind < N_grid) {
    ...
    v_d_b[ind] = v_d_CRemap[ind] + 0.5 * dt * (v_d_f_p0[ind] + v_d_f_p1[ind])
...
__global__ void y_Compact_operater_on_gpu(FLOAT *v_d_b, FLOAT *v_d_Cptb, INT N_grid, INT Ny) {
  ...
  while (ind < N_grid) {
    ...
    j_cell = ind % Ny;
    if(j_cell == 0){
        v_d_Cptb[ind] = (10 * v_d_b[i_cell*Ny + j_cell] + v_d_b[i_cell*Ny + j_cell+1]) / 12.0;
    }
    else if(j_cell == Ny-1)
    {
        v_d_Cptb[ind] = (10 * v_d_b[i_cell*Ny + j_cell] + v_d_b[i_cell*Ny + j_cell-1]) / 12.0;
        ...
        v_d_Cptb[ind] = (v_d_b[i_cell * Ny + (j_cell + 1)] + 10 * v_d_b[i_cell * Ny + j_cell] + v_d_b[i_cell * Ny + (j_cell - 1)]) / 12.0;
    }
    ind += gridDim.x * blockDim.x;
  }
  return;
...
int main(int argc, char *argv[]) {
  ...
  *************************************/
  ...
  FLOAT *v_d_C0, *v_d_C_xphf, *v_d_C0_Remap, *v_d_DC0_x_bar;                 // step 1
  FLOAT *v_d_IntC_y0, *v_d_IntC_y0_Remap, *v_d_DIntC_y0_bar, *v_d_IntC_yp1; // step 2
  FLOAT *v_d_IntC_xphf, *v_d_DIntC_xphf_bar, *v_d_Cp1, *v_d_C_Exact;       // step 3
  FLOAT *v_d_x, *v_d_y, *v_d_xhf, *v_d_yhf, *v_d_xhf_bar1, *v_d_yhf_bar2, *v_d_xhf_bar3;
  FLOAT *v_d_x_inter1, *v_d_y_inter2, *v_d_x_inter3;
  FLOAT *v_d_coA, *v_d_coB, *v_d_coC, *v_d_Cptb, *v_d_b, *p, *q;
  FLOAT *v_d_f1_p0, *v_d_f1_p1, *v_d_Int_f1_p0, *v_d_f2_p0, *v_d_f2_p1, *v_d_Int_f2_p0;
  ...
  *************************************************/
  ...
  cudaMalloc((void **)&v_d_Int_f1_p0, N_grid * sizeof(FLOAT));
  cudaMalloc((void **)&v_d_f2_p0, N_grid * sizeof(FLOAT));
  cudaMalloc((void **)&v_d_f2_p1, N_grid * sizeof(FLOAT));
  cudaMalloc((void **)&v_d_Int_f2_p0, N_grid * sizeof(FLOAT));
  ...
  //********************* step 1 **************************
  Intersection_point_vet_x<<<numBlocks, threadsPerBlock, 0, stream0>>>
  (v_d_xhf_bar1, v_d_yhf, v_d_y, v_d_x_inter1, Nx, Ny, N_points_inter1); 
  
  //********************* step 2 **************************
  Intersection_point_hori_y<<<numBlocks, threadsPerBlock, 0, stream1>>>
  (v_d_xhf, v_d_yhf_bar2, v_d_x, v_d_y_inter2, Nx, Ny, N_points_inter2); 
  
  //********************* step 3 **************************
  Intersection_point_vet_x<<<numBlocks, threadsPerBlock, 0, stream2>>>
  (v_d_xhf_bar3, v_d_yhf, v_d_y, v_d_x_inter3, Nx, Ny, N_points_inter1);   

  // Calculte Initial condition and Exact solution on CPU
  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock, 0, stream0>>>
  (v_d_x, v_d_y, v_d_C0, dx, dy, 0, K, N_grid);
  cudaMemcpyAsync(v_h_C0, v_d_C0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream0);

  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock, 0, stream1>>>
  (v_d_x, v_d_y, v_d_C_Exact, dx, dy, T_max, K, N_grid);
  cudaMemcpyAsync(v_h_C_Exact, v_d_C_Exact, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream1);
  ...
  for (itime = 1; itime <= Nt; itime++){
    ...
    cudaDeviceSynchronize();
    x_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock, 0, stream0>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_C0, v_d_C0_Remap, 
    v_d_xhf, v_d_x_inter1, N_grid, Nx, Ny, dx);

    //*************** Diffusion term ********************
    x_Diff_on_gpu<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C0, v_d_DC0_x_bar, v_d_xhf, v_d_x_inter1, 
    K, N_points_inter1, Nx, Ny, dx);

    // ************ source term at t^(n+1/2) ************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock, 0, stream0>>>
    (v_d_x, v_d_y, v_d_f1_p1, dx, dy, thf, K, N_grid);  
    cudaMemcpyAsync(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream0);
    sumf += sum(v_h_f, N_grid);

    // *************** source term at t^n ***************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_x, v_d_y, v_d_f1_p0, dx, dy, t0, K, N_grid);  
    x_Remap_on_gpu_part1<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, N_grid, Nx, Ny);
    x_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, v_d_Int_f1_p0, 
    v_d_xhf, v_d_x_inter1, N_grid, Nx, Ny, dx);
    cudaMemcpyAsync(v_h_f, v_d_Int_f1_p0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream1);
    sumf += sum(v_h_f, N_grid);
    ...
    (v_d_coA, v_d_coB, v_d_coC, v_d_b, v_d_C0_Remap, v_d_DC0_x_bar, v_d_Int_f1_p0, 
    v_d_f1_p1, N_grid, Nx, K, dt, dt_mul_dx_inverse, dt_mul_dx_square_inverse);
    cudaDeviceSynchronize();
    ...
    cudaDeviceSynchronize();
    y_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock, 0, stream0>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_C_xphf, v_d_IntC_y0,
    v_d_yhf, v_d_y_inter2, N_grid, Nx, Ny, dy);

    //**************** Diffusion term ********************
    y_Diff_on_gpu<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C_xphf, v_d_DIntC_y0_bar, v_d_yhf, v_d_y_inter2, 
    K, N_points_inter2, Nx, Ny, dy, dt_mul_dy_inverse);

    // ************ source term at t^(n+1) ***************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock, 0, stream0>>>
    (v_d_x, v_d_y, v_d_f2_p1, dx, dy, t1, K, N_grid);  
    cudaMemcpyAsync(v_h_f, v_d_f2_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream0);
    sumf += sum(v_h_f, N_grid);
    
    // *************** source term at t^n ****************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_x, v_d_y, v_d_f2_p0, dx, dy, t0, K, N_grid);  
    y_Remap_on_gpu_part1<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f2_p0, N_grid, Nx, Ny);
    y_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock, 0, stream1>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f2_p0, v_d_Int_f2_p0,
    v_d_yhf, v_d_y_inter2, N_grid, Nx, Ny, dy);
    cudaMemcpyAsync(v_h_f, v_d_Int_f2_p0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream1);
    sumf += sum(v_h_f, N_grid);
    ...
    (v_d_coA, v_d_coB, v_d_coC, v_d_b, v_d_IntC_y0, v_d_DIntC_y0_bar, v_d_f2_p1, 
    v_d_Int_f2_p0, N_grid, Ny, K, dt, dt_mul_dy_inverse, dt_mul_dy_square_inverse);
    cudaDeviceSynchronize();
    ...
    v_d_x_inter3, K, N_points_inter1, Nx, Ny, dx);

    // ************** source term at t^(n+1) ************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock, 0, stream0>>>
    (v_d_x, v_d_y, v_d_f1_p1, dx, dy, t1, K, N_grid);  
    cudaMemcpyAsync(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost, stream0);
    sumf += sum(v_h_f, N_grid);
    ...
    //*************** Matrix and vector******************
    x_Create_matrix_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_coA, v_d_coB, v_d_coC, v_d_b, v_d_IntC_xphf, v_d_DIntC_xphf_bar, v_d_Int_f1_p0, 
    v_d_f1_p1, N_grid, Nx, K, dt, dt_mul_dx_inverse, dt_mul_dx_square_inverse);
    cudaDeviceSynchronize();
    ...
    if (itime % T_span == 0) {
      ...
      cout << "time " << t1 << " Cmassdif " << Cmassdif << " Cmass_end " << cmass_end << endl;*/
    }
 ...
 // calculate error
  ...
  cudaFree(v_d_f1_p0);
  cudaFree(v_d_f1_p1);
  cudaFree(v_d_Int_f1_p0);
  cudaFree(v_d_f2_p0);
  cudaFree(v_d_f2_p1);
  cudaFree(v_d_Int_f2_p0);

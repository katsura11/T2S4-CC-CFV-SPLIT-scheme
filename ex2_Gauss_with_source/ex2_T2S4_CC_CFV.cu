...
inline FLOAT f(FLOAT x, FLOAT y, FLOAT t) { return 0; }
...
__global__ void x_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        ...
                                        FLOAT *v_d_f_p1, INT N_grid, INT Ny, FLOAT Kx, FLOAT dt, 
...
__global__ void y_Create_matrix_on_gpu(FLOAT *v_d_coA, FLOAT *v_d_coB, FLOAT *v_d_coC, FLOAT *v_d_b, 
                                        ...
                                        FLOAT *v_d_f_p1, INT N_grid, INT Ny, FLOAT Ky, FLOAT dt, 
...
int main(int argc, char *argv[]) {
  ...
  *************************************/
  ...
  FLOAT *v_d_IntC_xphf, *v_d_DIntC_xphf_bar, *v_d_Cp1;                     // step 3
 ...
 // Calculte Initial condition and Exact solution on CPU
  ...
  //********************* step 1 **************************
  Intersection_point_vet_x<<<numBlocks, threadsPerBlock>>>
 ...
 FLOAT sumf = 0.0;
  ...
  //for (itime = 1; itime <= 1; itime++){
    ...
    //*************** Diffusion term ********************
    x_Diff_on_gpu<<<numBlocks, threadsPerBlock>>>
    ...
    (v_d_x, v_d_y, v_d_f1_p1, t1-0.5*dt, Kx, N_grid);  
    //cudaMemcpy(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    cudaMemcpyAsync(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    sumf += sum(v_h_f, N_grid);

    // *************** source term at t^n ***************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_x, v_d_y, v_d_f1_p0, t0, Kx, N_grid);  
    x_Remap_on_gpu_part1<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, N_grid, Nx, Ny);
    x_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, v_d_Int_f1_p0, 
    v_d_xhf, v_d_x_inter1, N_grid, Nx, Ny, dx);
    //cudaMemcpy(v_h_f, v_d_Int_f1_p0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    cudaMemcpyAsync(v_h_f, v_d_Int_f1_p0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    sumf += sum(v_h_f, N_grid);

    //*************** Matrix and vector******************
    x_Create_matrix_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_coA, v_d_coB, v_d_coC, v_d_b, v_d_C0_Remap, v_d_DC0_x_bar, v_d_Int_f1_p0, 
    v_d_f1_p1, N_grid, Nx, Kx, dt, dt_mul_dx_inverse, dt_mul_dx_square_inverse);
    cudaDeviceSynchronize();
    ...
    //**************** Diffusion term ********************
    y_Diff_on_gpu<<<numBlocks, threadsPerBlock>>>
    ...
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_x, v_d_y, v_d_f2_p1, t1, Ky, N_grid);  
    //cudaDeviceSynchronize();
    //cudaMemcpy(v_h_f, v_d_f2_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    cudaMemcpyAsync(v_h_f, v_d_f2_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    sumf += sum(v_h_f, N_grid);
    
    // *************** source term at t^n ****************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_x, v_d_y, v_d_f2_p0, t0, Ky, N_grid);  
    y_Remap_on_gpu_part1<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f2_p0, N_grid, Nx, Ny);
    y_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f2_p0, v_d_Int_f2_p0,
    v_d_yhf, v_d_y_inter2, N_grid, Nx, Ny, dy);
    cudaMemcpyAsync(v_h_f, v_d_Int_f2_p0, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    sumf += sum(v_h_f, N_grid);

    //*************** Matrix and vector******************
    y_Create_matrix_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_coA, v_d_coB, v_d_coC, v_d_b, v_d_IntC_y0, v_d_DIntC_y0_bar, v_d_f2_p1, 
    v_d_Int_f2_p0, N_grid, Ny, Ky, dt, dt_mul_dy_inverse, dt_mul_dy_square_inverse);
    cudaDeviceSynchronize();
    ...
    // ************** source term at t^(n+1) ************
    get_source_term_on_gpu<<<numBlocks, threadsPerBlock>>>
    (v_d_x, v_d_y, v_d_f1_p1, t1, Kx, N_grid);  
    //cudaMemcpy(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    cudaMemcpyAsync(v_h_f, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToHost);
    sumf += sum(v_h_f, N_grid);
    ...
    // ************ source term at t^(n+1/2) *************
    //v_d_f1_p0 = v_d_f1_p1; 
    cudaMemcpyAsync(v_d_f1_p0, v_d_f1_p1, N_grid * sizeof(FLOAT), cudaMemcpyDeviceToDevice);
    x_Remap_on_gpu_part1<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, N_grid, Nx, Ny);
    x_Remap_on_gpu_part2<<<numBlocks, threadsPerBlock>>>
    (v_d_CL, v_d_CR, v_d_C6, v_d_Dltc, v_d_f1_p0, v_d_Int_f1_p0, 

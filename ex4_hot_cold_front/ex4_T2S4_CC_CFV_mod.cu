...
/************************************ functions ************************************/
__host__ __device__ FLOAT Vx(FLOAT x, FLOAT y) { 
  FLOAT r = sqrt(x*x + y*y);
  FLOAT f_t = tanh(r)/(cosh(r) * cosh(r));
  if (r == 0) return 0;
  else return -(y/r) * (f_t/0.385); 
}
__host__ __device__ FLOAT Vy(FLOAT x, FLOAT y) { 
  FLOAT r = sqrt(x*x + y*y);
  FLOAT f_t = tanh(r)/(cosh(r) * cosh(r));
  if (r == 0) return 0;
...
__host__ __device__ FLOAT C_Exact(FLOAT x, FLOAT y, FLOAT t) {
    FLOAT r = sqrt(x*x + y*y);
    FLOAT f_t = tanh(r)/(cosh(r) * cosh(r));
    if (r == 0) return 0;
...
__global__ void Get_exact_solution_on_gpu(FLOAT *v_d_x, FLOAT *v_d_y, FLOAT *v_C_Exact, 
                                          FLOAT dx, FLOAT dy, FLOAT t, INT N_grid) 
{
  ...
  while (tid < N_grid) {
    ...
    v_C_Exact[tid] = 1.0/ 36.0 * 
                    (C_Exact(x_mhf, y_mhf, t) + C_Exact(x_phf, y_mhf, t) + 
                     C_Exact(x_mhf, y_phf, t) + C_Exact(x_phf, y_phf, t) + 
                     4.0 * ((C_Exact(x_mhf, y, t) + C_Exact(x_phf, y, t) + 
                            C_Exact(x, y_mhf, t) + C_Exact(x, y_phf, t))) + 
                     16.0 * C_Exact(x, y, t));
                                          ...
                                          INT N_points, FLOAT dx, FLOAT dy, FLOAT dt) 
{
  ...
  while (tid < N_points) {
    ...
    v_d_xhf_bar1[tid] = v_d_xhf[tid] - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid]) + Vx(x_star1, v_d_yhf[tid]));

    /**************************************** step 2 *******************************************/
    y_star2 = v_d_yhf[tid] -       dt * Vy(v_d_xhf[tid], v_d_yhf[tid]);
    v_d_yhf_bar2[tid] = v_d_yhf[tid] -  0.5 * dt * (Vy(v_d_xhf[tid], v_d_yhf[tid]) + Vy(v_d_xhf[tid], y_star2));    
    
    /**************************************** step 3 *******************************************/
    x_star3 = v_d_xhf[tid] - 0.5 * dt * Vx(v_d_xhf[tid], v_d_yhf[tid]);
    v_d_xhf_bar3[tid] = v_d_xhf[tid] - 0.25 * dt * (Vx(v_d_xhf[tid], v_d_yhf[tid]) + Vx(x_star3, v_d_yhf[tid]));
                                      ...
                                      FLOAT *v_d_y, FLOAT *v_d_C, INT N_grid, INT Nx, INT Ny, FLOAT t) 
{
  ...
  while (ind < N_grid) {
    ...
    bL = C_Exact(v_d_x[0], v_d_y[ind], t);
    ...
    bR = C_Exact(v_d_x[N_grid - 1], v_d_y[ind], t);
                                      ...
                                      FLOAT *v_d_y, FLOAT *v_d_C, INT N_grid, INT Nx, INT Ny, FLOAT t) 
{
  ...
  while (ind < N_grid) {
    ...
    //************************ calculate ghost cell values ************************
    bB = C_Exact(v_d_x[ind], v_d_y[0], t);
    ...
    bT = C_Exact(v_d_x[ind], v_d_y[Ny-1], t);
...
int main(int argc, char *argv[]) {
  ...
  *************************************/
  ...
  FLOAT *v_d_IntC_xphf, *v_d_C_Exact;                   // step 3
  ...
  *************************************************/
  ...
  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock>>>(v_d_x, v_d_y, v_d_C0, dx, dy, 0, N_grid);
  Get_exact_solution_on_gpu<<<numBlocks, threadsPerBlock>>>(v_d_x, v_d_y, v_d_C_Exact, dx, dy, T_max, N_grid);
